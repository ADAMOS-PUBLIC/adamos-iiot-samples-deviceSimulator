...
def getNextLine(file, source):
    line = readLine(file)
    line=line.replace('$source', source)
    line=line.replace('$time', datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z')
...
def startMeasurements():
    if os.path.isfile('measurements.json') == False:
        return
    measurementFile=open('measurements.json','rU')
    while True:
        measurement = getNextLine(measurementFile, deviceId)
...
def startEvents():
    if os.path.isfile('events.json') == False:
        return
    eventsFile=open('events.json','rU')
    while True:
        event = getNextLine(eventsFile, deviceId)
        sendEvent(event['event'])
        time.sleep(event['wait'])

def startAlarms():
    if os.path.isfile('alarms.json') == False:
        return
    currentActiveAlarms = {}
    alarmsFile=open('alarms.json','rU')
    while True:
        alarm = getNextLine(alarmsFile, deviceId)
